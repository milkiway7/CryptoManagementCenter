...
export const TabChart = ({ symbol }) => {
    ...
    useEffect(() => {
        ...
        fetch(`api/crypto/linechart?symbol=${symbol}&interval=${interval}&startTime=${startTime}`, {
            ...
            if (!response.ok) {
                throw new Error("Can't fetch data for line chart from binance");
            }
    ...
    useEffect(() => {
        fetch(`api/crypto/trades?symbol=${symbol}`, {
            ...
            if (!response.ok) {
                throw new Error("Can't fetch data for recent trades from binance")
            }
            return response.json();
        }).then((data) => {
            console.log(data)
